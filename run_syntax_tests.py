...
class ProfileSyntaxDefinitionCommand(sublime_plugin.WindowCommand):
    def run(self, **kwargs):
        ...
        if not view:
            sublime.error_message(
                "Syntax performance tests can only be run when a buffer is open"
            )
            return
        ...
        if num > 0:
            ...
            for pattern in sorted(patterns, key=lambda p: p[0]):
                ...
                if next_char in {"'", '"'}:
                    ...
                    escaped_quotes = self.count_escapes(
                        next_char, view, chunk_begin, chunk_end
                    )
                ...
                append(
                    self.output_view,
                    line_pattern.format(relative_path, line + 1, col + 1, pattern[2]),
                )
...
def append(panel, output):
    panel.run_command(
        "append", {"characters": output, "force": True, "scroll_to_end": True}
    )
